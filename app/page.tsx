...
const loginSchema = z.object({
  ...
  password: z.string().min(8, "Password must include uppercase, lowercase, number, and special character").regex(
      /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])/,
...
export default function Home() {
  ...
  const {register, formState:{errors}, handleSubmit} = useForm<Inputs>({resolver: zodResolver(loginSchema)});
      ...
      <div className="flex-shrink-0 flex lg:flex-row max-w-[1440px] mx-auto w-full  gap-x-24">
        ...
        <div className="max-w-md w-full flex flex-col items-center justify-center gap-4 py-20">
          ...
          <div className="formField w-full">
            ...
            <ClerkProvider>
                <Button
                className="bg-blue-500 text-white w-full pu-2.5 rounded-md hover:bg-none"
                type="submit"
              >
                Sign In
              </Button>
